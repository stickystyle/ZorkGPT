...
There is a small mailbox here."""
        ...
        self.assertFalse(extraction.in_combat, "Should not detect combat in peaceful scenario")
    ...
    def test_combat_message_variations(self):
        ...
        for message, expected_combat in combat_variations:
            with self.subTest(message=message):
                ...
                self.assertIsNotNone(extraction, f"Extraction should not be None for: {message}")
                self.assertEqual(extraction.in_combat, expected_combat, 
    ...
    def test_non_combat_variations(self):
        ...
        for message in non_combat_variations:
            with self.subTest(message=message):
                extraction = self.agent.get_extracted_info(message)
                self.assertIsNotNone(extraction, f"Extraction should not be None for: {message}")
                self.assertFalse(extraction.in_combat, 
                               f"Should not detect combat for: {message}")
