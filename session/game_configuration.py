    ...
    def from_toml(cls, config_file: Optional[Path] = None) -> "GameConfiguration":
        ...
        def require_key(config_dict: dict, key: str, section: str) -> any:
            ...
            if key not in config_dict:
                raise KeyError(f"Missing required key '{key}' in [tool.zorkgpt.{section}] section")
            return config_dict[key]
        
        return cls(
            # Core game settings
            max_turns_per_episode=require_key(orchestrator_config, "max_turns_per_episode", "orchestrator"),
            turn_delay_seconds=require_key(gameplay_config, "turn_delay_seconds", "gameplay"),
            ...
            zork_game_workdir=require_key(gameplay_config, "zork_game_workdir", "gameplay"),
            ...
            # Update intervals
            knowledge_update_interval=require_key(orchestrator_config, "knowledge_update_interval", "orchestrator"),
            map_update_interval=require_key(orchestrator_config, "map_update_interval", "orchestrator"),
            objective_update_interval=require_key(orchestrator_config, "objective_update_interval", "orchestrator"),
            
            # Objective refinement
            enable_objective_refinement=require_key(orchestrator_config, "enable_objective_refinement", "orchestrator"),
            objective_refinement_interval=require_key(orchestrator_config, "objective_refinement_interval", "orchestrator"),
            max_objectives_before_forced_refinement=require_key(orchestrator_config, "max_objectives_before_forced_refinement", "orchestrator"),
            refined_objectives_target_count=require_key(orchestrator_config, "refined_objectives_target_count", "orchestrator"),
            
            # Context management
            max_context_tokens=require_key(orchestrator_config, "max_context_tokens", "orchestrator"),
            context_overflow_threshold=require_key(orchestrator_config, "context_overflow_threshold", "orchestrator"),
            
            # State export
            enable_state_export=require_key(orchestrator_config, "enable_state_export", "orchestrator"),
            s3_key_prefix=require_key(aws_config, "s3_key_prefix", "aws"),
            
            # Gameplay settings
            critic_rejection_threshold=require_key(gameplay_config, "critic_rejection_threshold", "gameplay"),
        )
