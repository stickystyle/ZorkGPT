...
class MovementAnalyzer:
    ...
    def _check_pending_resolution(self, context: MovementContext) -> Optional[MovementResult]:
        """Check if current context resolves a pending connection"""
        for i, pending in enumerate(self.pending_connections[:]):  # Copy for safe removal
            if self._should_resolve_pending(pending, context):
                ...
                return MovementResult(
                    ...
                    is_pending=False,
                    environmental_factors=self._detect_environmental_factors(context.game_response),
    ...
    def _analyze_new_movement(self, context: MovementContext) -> MovementResult:
        ...
        if (context.current_location != context.previous_location and
            context.current_location and
            context.current_location.lower() not in GENERIC_LOCATION_FALLBACKS and
        ...
        elif self._indicates_pending_movement(context.game_response, context.current_location, context.previous_location):
    ...
    def _should_resolve_pending(self, pending: PendingConnection, context: MovementContext) -> bool:
    ...
    def _indicates_pending_movement(self, response: str, current_location: str, previous_location: str) -> bool:
        """
        ...
        is_dark_response = any(indicator in response_lower for indicator in dark_room_indicators)
        
        # Also check if location extraction failed (stayed same) which might indicate unclear destination
        location_unclear = (current_location == previous_location and 
    ...
    def _detect_environmental_factors(self, response: str) -> List[str]:
        ...
        if any(dark in response_lower for dark in ["dark", "pitch", "grue", "darkness"]):
            factors.append("darkness")
            
        # Locked/blocked factors
        if any(locked in response_lower for locked in ["locked", "bolt", "key", "closed"]):
            factors.append("locked")
            
        # Physical barriers
        if any(blocked in response_lower for blocked in ["wall", "barrier", "blocked", "impassable"]):
            factors.append("blocked")
            
        # Climbing/elevation factors
        if any(climb in response_lower for climb in ["ladder", "climb", "rope", "steep"]):
            factors.append("climbing")
            
        # Water/swimming factors
        if any(water in response_lower for water in ["water", "swim", "river", "stream"]):
            factors.append("water")
            
        # Combat/danger factors
        if any(combat in response_lower for combat in ["troll", "attack", "fight", "hostile"]):
            factors.append("combat")
            
        # Tool/item requirement indicators
        if any(tool in response_lower for tool in ["need", "require", "must have", "without"]):
...
def create_movement_context(current_location: str, previous_location: Optional[str], 
                          action: str, game_response: str, turn_number: int) -> MovementContext:
