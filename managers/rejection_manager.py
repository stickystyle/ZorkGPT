...
class RejectionManager(BaseManager):
    ...
    def update_trust(self, was_rejection_correct: bool) -> None:
        ...
        if len(self.state.recent_outcomes) >= 5:
            recent_accuracy = sum(self.state.recent_outcomes) / len(self.state.recent_outcomes)
    ...
    def should_override_rejection(
        ...
        turns_without_progress = context.get("turns_since_movement", self.state.turns_since_movement)
        ...
        for action_taken, response in previous_actions_and_responses[-5:]:
            ...
            if any(phrase in response_lower for phrase in [
                "i don't understand", "you can't", "that's not", "invalid",
        ...
        if (turns_without_progress > 8 and actual_failures > 2 and 
            unique_recent_locations == 1 and action_frequency <= 1):
        ...
        if len(recent_locations) >= 4:
            # A-B-A-B pattern
            if (recent_locations[-4] == recent_locations[-2] and 
                recent_locations[-3] == recent_locations[-1] and
                recent_locations[-4] != recent_locations[-3]):
    ...
    def add_rejected_action(self, action: str, score: float, justification: str) -> None:
