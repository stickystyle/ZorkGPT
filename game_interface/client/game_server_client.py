...
class GameServerClient:
    ...
    def start(self, session_id: Optional[str] = None) -> str:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/sessions/{self.session_id}", timeout=self.timeout
            )
    ...
    def force_save(self) -> bool:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/sessions/{self.session_id}/save", timeout=self.timeout
            )

            if response.status_code == 200:
                data = response.json()
                logger.info(
                    f"Force save successful for session {self.session_id} at turn {data.get('turn')}"
                )
                return True
            else:
