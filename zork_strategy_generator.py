...
class AdaptiveKnowledgeManager:
    ...
    def __init__(self, log_file: str = "zork_episode_log.jsonl", output_file: str = "knowledgebase.md"):
    ...
    def update_knowledge_from_turns(self, episode_id: str, start_turn: int, end_turn: int) -> bool:
        """
        ...
        quality_score, quality_reason = self._assess_knowledge_update_quality(turn_data)
        print(f"  📊 Knowledge quality score: {quality_score:.1f}/10 - {quality_reason}")
        
        if quality_score < self.min_quality_threshold:
            print(f"  ⏭️ Skipping update - quality below threshold ({self.min_quality_threshold})")
            return False
    ...
    def _extract_turn_window_data(self, episode_id: str, start_turn: int, end_turn: int) -> Optional[Dict]:
            ...
            with open(self.log_file, "r", encoding="utf-8") as f:
                ...
                for line in f:
                    try:
                        ...
                        elif event_type == "zork_response" and turn_data["actions_and_responses"]:
                            # Update the last action with its response
                            turn_data["actions_and_responses"][-1]["response"] = log_entry.get("zork_response", "")
                        ...
                        # Track location changes
                            ...
                            if new_location and new_location != current_location and new_location != "Unknown Location":
    ...
    def _assess_knowledge_update_quality(self, turn_data: Dict) -> Tuple[float, str]:
        ...
        actions_summary = "\n".join([
            ...
            if len(action['response']) > 100 else f"Turn {action['turn']}: {action['action']} -> {action['response']}"
...
Focus on whether this data would generate actionable, non-repetitive insights.
...
Respond with just the strategy name: FULL_UPDATE, SELECTIVE_UPDATE, CONSOLIDATION_ONLY, or ESCAPE_ANALYSIS"""
        ...
        try:
            ...
            # Validate strategy
            valid_strategies = ["FULL_UPDATE", "SELECTIVE_UPDATE", "CONSOLIDATION_ONLY", "ESCAPE_ANALYSIS"]
    ...
    def _perform_targeted_analysis(self, turn_data: Dict, strategy: str) -> Optional[str]:
    ...
    def _analyze_selective_insights(self, turn_data: Dict) -> Optional[str]:
        ...
        actions_text = ""
        for action in turn_data["actions_and_responses"][:20]:  # Limit to avoid token overflow
...
Do not add new information - only reorganize and clarify existing knowledge."""
    ...
    def _intelligent_knowledge_merge(self, new_insights: str, strategy: str) -> bool:
        ...
        if strategy == "CONSOLIDATION_ONLY":
            ...
            # Intelligent merge
            merged_knowledge = self._merge_insights_with_existing(existing_knowledge, new_insights, strategy)
    ...
    def _merge_insights_with_existing(self, existing: str, new_insights: str, strategy: str) -> Optional[str]:
